...
async function problemA() {
  ...
  // AsyncAwait version
  const namesPoems = ['poem-two/stanza-01.txt', 'poem-two/stanza-02.txt'];
  const promisesArr = namesPoems.map((name) => promisifiedReadFile(name));
  const poems = await Promise.all(promisesArr)
...
async function problemB() {
  ...
  // AsyncAwait version
  const promisesArr = filenames.map((name) => promisifiedReadFile(name));
  const poems = await Promise.all(promisesArr)
  for (const poem of poems) {
    blue(poem)
  }
