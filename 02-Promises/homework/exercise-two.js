...
function problemA () {
  ...
  // promise version
  //Promise.all() recibe un arreglo de promesas que se atan a una promesa global
  //La promesa global no se ejecuta hasta que se ejecuten todas las promesas pasadas a all()
  Promise.all(
    [
      promisifiedReadFile('poem-two/stanza-01.txt'),
      promisifiedReadFile('poem-two/stanza-02.txt')
    ]
  ) //Cada solución de las promesas se guarda en un arreglo que se pasa al then
  .then(
    (stanzas)=>{
      blue(stanzas[0]);
      blue(stanzas[1]);
